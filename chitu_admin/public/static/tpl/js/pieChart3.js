...
$(document).ready(function() {
	...
	// 3具体承运商班次年承接订单重量（年月一个页面）
    var percarriershiftYearWeight = '/backstage/statistics/yeartotaltransport';
    ...
    var optionFour = {
        ...
        xAxis : [
            {
                type : 'category',
                data : ['北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海','北京-上海']
            }
        ],
        yAxis : [
            {
 ...
 //        })
	// }
	// 3具体承运商班次年承接订单重量
	function getPercarriershiftYearWeight(year,month) {
		var data = {"year":year,"month":month};
        $.post(percarriershiftYearWeight,data,function(response){
            var resObj = JSON.parse(response);
            console.log(resObj);
            var arrX = new  Array();
            var arrD1 = new  Array();
            var arrD2 = new  Array();
            $.each(resObj,function(index,value){
                arrX[index] = value.shiftName;
                arrD1[index] = parseInt(value.weight); 
                arrD2[index] = parseInt(value.sumweight);
            });
            optionFour['xAxis'][0].data = arrX;
            optionFour['series'][0].data = arrD1;
            optionFour['series'][1].data = arrD2;
