...
$(document).ready(function() {
    // 当年每月每个业务员订单数量
    var businessOrderURL = '/admin/charts/businessOrder';
    // 当年每月每个业务员订单数量
    var businessOrderPieURL = '/admin/charts/businessOrderPie';
    // 当年每月每个业务员订单金额
    var businessMoneyPieURL = '/admin/charts/businessMoneyPie';
    ...
    // 初始化业务员订单数量柱状图
    var echartOne = echarts.init(document.getElementById('echartOne'));
    // 请求
    send(businessOrderURL,'',function (response) {
        // 创建数组
        var data = new Array();
        // 遍历数组
        ...
        // 插入图标数组
        options.bar['legend']['data'] = data;
        options.bar['series'] = response;
        // setoption
        echartOne.setOption(options.bar,{
            notMerge: true
        });
    });
    ...
    setTimeout(function (argument) {
        // 初始化业务员订单数量饼图
        var echartTwo = echarts.init(document.getElementById('echartTwo'));
        // 请求
        send(businessOrderPieURL,'',function (response) {
            // 创建数组
            var data = new Array();
            // 遍历数组
            for (var i = 0; i < response.length; i++) {
            ...
            // 插入图标数组
            options.pie['legend']['data'] = data;
            options.pie['series'][0]['data'] = response;
            // setoption
            echartTwo.setOption(options.pie,{
                notMerge: true
            });
        });
    },1000);
    ...
    setTimeout(function (argument) {
        // 初始化业务员订单金额饼图
        var echartThree = echarts.init(document.getElementById('echartThree'));
        // 请求
        send(businessMoneyPieURL,'',function (response) {
            // 创建数组
            var data = new Array();
            // 遍历数组
            for (var i = 0; i < response.length; i++) {
            ...
            // 插入图标数组
            options.pie2['legend']['data'] = data;
            options.pie2['series'][0]['data'] = response;
            // setoption
            echartThree.setOption(options.pie2,{
                notMerge: true
            });
        });
    },2000);
