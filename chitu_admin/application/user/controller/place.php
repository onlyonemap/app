...
//                  $condition['g.status'] = ['IN','1,3'];
            // 查询条件 已开通城市 线路ID
            $condition['linecityid'] = $result['city_id'];
            $condition['whethertoopen'] = 1;
            if (!empty($start_area)){
                $condition['beginareaid'] = $start_area;
            }
            if ( !empty($end_area)){
                $condition['endareaid'] = $end_area;
            }
            // 起点城市
            $start_city = addresidToName($start_id);
            // 终点城市
            ...
            if(!empty($result)){
                $line = Db::table("ct_shift")
                    ->field('sid,datatime,begincityid,beginareaid,endareaid,endcityid,transit,istransit,picktype,sendtype,picksite,stime,sphone,sendsite,dtime,tphone,companyid,shiftnumber,trunkaging,price,timestrat,timeend,lowprice,shiftstate,pmoney,smoney,weekday,discount')
                    ->where($condition)
//                          ->order($order)
                    ->select();
                    ...
                    if (empty($startline) ||empty($endline)){
                        ...
                        for ($i = 0;$i<count($start_line);$i++){
                            for ($j = 0;$j<count($end_line);$j++){
                                if ($start_line[$i]['endcityid'] === $end_line[$j]['begincityid']){
...
//            $condition1['g.status'] = ['IN','1,3'];
            ...
            if(!empty($result)){
                ...
                if(empty($end_id)){
                    foreach ($result as $key =>$value){
                        $line1 = Db::table("ct_shift")
                            ->field('sid,whethertoopen,datatime,begincityid,endcityid,beginareaid,endareaid,transit,istransit,picktype,sendtype,picksite,stime,sphone,sendsite,dtime,tphone,companyid,shiftnumber,trunkaging,price,timestrat,timeend,lowprice,shiftstate,pmoney,smoney,weekday,discount')
                            ->where($condition)
                            ->where('linecityid',$value)
                            ->order($order)
                            ->select();
...
//                    $list[$key]['deptime'] = $value['deptime'];
                ...
                $list[$key]['datatime'] = $value['datatime'];
...
//                    }
    ...
    public function datalist(){
        ...
        if ($level == 1){
            ...
            $start_area = input('start_city');
            $end_area = input('end_city');
            $start_city = areatocity(input('start_city'));
            $end_city = areatocity(input('end_city'));
        }
...
//		$data['city'] = $this->city_price($start_city, $end_city);
    ...
    public function saveconnect($startline,$endline,$transit,$companyname,$ShiftNumber,$condition1,$order){
        ...
        $newline['datatime'] = array_intersect($startcity['datatime'],$endcity['datatime']);
...
//         exit();
        ...
        if(!empty($insert)){
            ...
            $line = Db::table("ct_shift")
                ->alias('s')
                ->join('__SHIFT_LOG__ g','g.shiftid = s.sid')
                ->field('s.sid,s.begincityid,s.endcityid,s.transit,s.istransit,s.picktype,s.sendtype,s.picksite,s.stime,s.sphone,s.sendsite,s.dtime,s.tphone,s.companyid,s.shiftnumber,s.trunkaging,s.price,s.timestrat,s.timeend,s.lowprice,s.shiftstate,s.pmoney,s.smoney,s.weekday,s.discount,g.slid,g.deptime')
                ->where('sid',$insert)
                ->where($condition1)
                ->order($order)
                ->select();
